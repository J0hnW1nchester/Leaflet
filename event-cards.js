...
document.addEventListener('DOMContentLoaded', () => {
  ...
  eventsContainer.addEventListener('click', (e) => {
    ...
    // Prevent scrolling on the body, but allow scroll in description-container
    const preventScroll = (e) => {
      if (e.target.closest('.description-container')) {
        return; // Allow scroll inside description-container
      }
      e.preventDefault(); // Prevent scroll on body or overlay
    };
    ...
    closeButton.addEventListener('click', () => {
      // Remove the scroll blockers when the popup is closed
      document.removeEventListener('wheel', preventScroll);
      document.removeEventListener('touchmove', preventScroll);
      document.removeEventListener('keydown', preventScroll);
